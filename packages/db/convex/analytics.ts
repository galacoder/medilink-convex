...
export const getProviderSummary = query({
  ...
  handler: async (ctx, args) => {
    ...
    const completedSRs = serviceRequests.filter(
      (sr) => sr.status === "completed",
    );
    const cancelledSRs = serviceRequests.filter(
    ...
    const revenueQuotes = acceptedQuotes.filter((q) =>
      completedSRIds.has(q.serviceRequestId as Id<"serviceRequests">),
    );
    ...
    const thisMonthSRs = allServiceRequests.filter(
      (sr) =>
        sr.createdAt >= thisMonthStart.getTime() && sr.status === "completed",
    );
