...
describe("disputes.create", () => {
  ...
  it("test_create_creates_audit_entry", async () => {
    ...
    const auditLogs = await t.run(async (ctx) =>
      ctx.db
        .query("auditLog")
        .filter((q) => q.eq(q.field("action"), "dispute.created"))
        .collect(),
    );
...
describe("disputes.updateStatus", () => {
  ...
  it("test_updateStatus_updates_updatedAt", async () => {
    ...
    const disputeBefore = (await t.run(async (ctx) =>
      ctx.db.get(disputeId as any),
    )) as any;
    const before = Date.now();
    ...
    const disputeAfter = (await t.run(async (ctx) =>
      ctx.db.get(disputeId as any),
    )) as any;
...
describe("disputes.escalate", () => {
  ...
  it("test_escalate_creates_audit_entry", async () => {
    ...
    const auditLogs = await t.run(async (ctx) =>
      ctx.db
        .query("auditLog")
        .filter((q) => q.eq(q.field("action"), "dispute.escalated"))
        .collect(),
    );
    expect(auditLogs.length).toBeGreaterThan(0);
  });
...
describe("disputes.resolve", () => {
  it("test_resolve_sets_resolved_status_and_resolvedAt", async () => {
    ...
    expect(dispute!.resolutionNotes).toBe(
      "Vấn đề đã được giải quyết sau khi xem xét kỹ.",
    );
  });
