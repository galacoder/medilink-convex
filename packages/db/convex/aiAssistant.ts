...
Respond ONLY with valid JSON.`;
    ...
    try {
      ...
      const summary =
        parsed.summary ?? `Tìm kiếm: ${args.query} (Search: ${args.query})`;
      const suggestions = [
...
- low: routine maintenance, cosmetic issues`;
    ...
    try {
      ...
      return {
        ...
        descriptionSuggestion: parsed.descriptionSuggestion ?? args.description,
  ...
  "suggestions": ["follow-up action 1", "follow-up action 2"]
    ...
    try {
      ...
      return {
        ...
        suggestions: [
          "Thử lại (Try again)",
          "Xem bảng phân tích (View analytics)",
        ],
      };
    }
  },
...
export const deleteConversation = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!conversation) {
      throw new ConvexError({
        message: "Hội thoại không tồn tại. (Conversation not found.)",
        code: "NOT_FOUND",
      });
    }
