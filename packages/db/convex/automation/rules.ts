...
export const checkOverdueRequests = internalMutation({
  ...
  handler: async (ctx) => {
    ...
    for (const request of allRequests) {
      const isStalled = (stalledStatuses as readonly string[]).includes(
        request.status,
      );
    ...
    await ctx.runMutation(
      internal.automation.automationLog.recordAutomationRun,
      {
...
export const checkMaintenanceDue = internalMutation({
  ...
  handler: async (ctx) => {
    ...
    for (const record of scheduledRecords) {
      const isDueSoon =
        record.scheduledAt >= now && record.scheduledAt <= dueSoonThreshold;
      if (isDueSoon) {
        totalAffected++;
        dueDetails.push({
    ...
    await ctx.runMutation(
      internal.automation.automationLog.recordAutomationRun,
      {
        ...
        metadata:
          dueDetails.length > 0
            ? { dueMaintenance: dueDetails.slice(0, 10) }
            : undefined,
      },
    );
      ...
      internal.automation.automationLog.recordAutomationRun,
      {
        ...
        metadata:
          lowStockDetails.length > 0
            ? { lowStock: lowStockDetails.slice(0, 10) }
            : undefined,
      },
    );
  ...
  handler: async (ctx) => {
    ...
    for (const cert of allCertifications) {
      ...
      const isExpiringSoon =
        cert.expiresAt >= now && cert.expiresAt <= expiryWindow;
      if (isExpiringSoon) {
        totalAffected++;
        expiryDetails.push({
      ...
      internal.automation.automationLog.recordAutomationRun,
      {
        ...
        metadata:
          expiryDetails.length > 0
            ? { expiringCerts: expiryDetails.slice(0, 10) }
            : undefined,
      },
    );
