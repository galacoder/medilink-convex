...
/**
 * Hospital members management page.
 *
 * WHY: Hospital owners and admins need to invite new members, view the
 * full members list, change roles, and remove members. This page composes
 * InviteMemberForm + MembersTable with the hospital org context.
 *
 * Data: Better Auth's useActiveOrganization() provides the members list
 * (real-time via Better Auth sync). Role changes and removals route
 * through the tRPC/fetch layer.
 */
export default function HospitalMembersPage() {
  ...
  // WHY: We derive callerRole from members array to avoid extra query
  const callerRole: OrgRole = "member"; // Default; actual check via session
  const callerId = "";
