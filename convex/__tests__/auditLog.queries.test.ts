...
describe("auditLog.list", () => {
  it("test_list_returns_all_entries_for_platform_admin", async () => {
    ...
    const adminId = await seedUser(t, "admin@sangtech.com", "Platform Admin", "platform_admin");

    await seedAuditLog(t, org1Id, adminId);
    await seedAuditLog(t, org2Id, adminId, { action: "dispute.created", resourceType: "disputes" });
  ...
  it("test_list_throws_when_unauthenticated", async () => {
    const t = convexTest(schema, modules);
    await expect(
      t.query(api.admin.auditLog.list, {}),
    ).rejects.toThrow();
  });
  ...
  it("test_list_throws_when_not_platform_admin", async () => {
    ...
    await expect(
      asStaff.query(api.admin.auditLog.list, {}),
    ).rejects.toThrow();
  });
  ...
  it("test_list_filters_by_resource_type", async () => {
    const t = convexTest(schema, modules);
    const orgId = await seedOrganization(t);
    const adminId = await seedUser(t, "admin@sangtech.com", "Admin", "platform_admin");
    ...
    expect(result.entries).toHaveLength(2);
    expect(result.entries.every((e: any) => e.resourceType === "disputes")).toBe(true);
  });
  ...
  it("test_list_filters_by_organization", async () => {
    ...
    expect(result.entries).toHaveLength(2);
    expect(result.entries.every((e: any) => e.organizationId === org1Id)).toBe(true);
  });
...
describe("auditLog.exportCSV", () => {
  it("test_exportCSV_returns_csv_string_with_headers", async () => {
    ...
    const asAdmin = t.withIdentity({ subject: adminId, platformRole: "platform_admin" });
    const csv = await asAdmin.query(api.admin.auditLog.exportCSV, {}) as string;
  ...
  it("test_exportCSV_includes_data_rows", async () => {
    ...
    await seedAuditLog(t, orgId, adminId, { action: "equipment.status_changed" });
