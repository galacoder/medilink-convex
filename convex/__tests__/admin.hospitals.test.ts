...
describe("platformAdmin.listHospitals", () => {
  it("test_listHospitals_returns_all_hospitals", async () => {
    ...
    const adminUserId = await seedUser(t, "admin@sanglertech.com", "platform_admin");
...
describe("platformAdmin.onboardHospital", () => {
  it("test_onboardHospital_creates_hospital_org", async () => {
    ...
    const result = await asAdmin.mutation(api.admin.hospitals.onboardHospital, {
      name: "New Hospital",
      slug: "new-hospital",
...
describe("platformAdmin.suspendHospital", () => {
  it("test_suspendHospital_sets_status_to_suspended", async () => {
    const t = convexTest(schema, modules);
    const adminUserId = await seedUser(t, "admin@sanglertech.com", "platform_admin");
    const hospitalId = await seedOrganization(t, "Active Hospital", "hospital", {
      status: "active",
    });
    ...
    const result = await asAdmin.mutation(api.admin.hospitals.suspendHospital, {
      hospitalId: hospitalId as any,
...
describe("platformAdmin.reactivateHospital", () => {
  it("test_reactivateHospital_sets_status_to_active", async () => {
    const t = convexTest(schema, modules);
    const adminUserId = await seedUser(t, "admin@sanglertech.com", "platform_admin");
    const hospitalId = await seedOrganization(t, "Suspended Hospital", "hospital", {
      status: "suspended",
    });
